...
pub fn write_significance_results(results: &[SignificanceResult], path: &Path) -> Result<()> {
    ...
    for result in results {
        wtr.write_record(&[
            ...
            &result.raw.variant.resistant_ref_depth.to_string(),
            &result.raw.variant.resistant_alt_depth.to_string(),
            &result.raw.variant.resistant_dp.to_string(),
            &result.raw.variant.resistant_gq.to_string(),
            &result.raw.variant.susceptible_ref_depth.to_string(),
            &result.raw.variant.susceptible_alt_depth.to_string(),
            &result.raw.variant.susceptible_dp.to_string(),
            &result.raw.variant.susceptible_gq.to_string(),
            &format!("{:.6}", result.raw.g_statistic),
            &format!("{:.6}", result.raw.snp_index_resistant),
            &format!("{:.6}", result.raw.snp_index_susceptible),
